...
import {
  ...
  Shield, Lock, ChevronRight, Loader2, IndianRupee,
} from "lucide-react";
...
export default function Payment() {
  ...
  const canProceed =
    tab === "upi"
      ? (step === "select" ? !!selectedUpi : upiId.trim().includes("@"))
      : tab === "card"
  ...
  const handlePay = () => {
    ...
    setTimeout(() => {
      ...
      addStoredNotif({
        ...
        body: `₹${fare.total.toLocaleString()} paid via ${methodLabel} for ${vehicleName} · ${route}`,
        requestId: id,
      });
  ...
  if (step === "success") {
    return (
      <div className="max-w-lg mx-auto bg-white min-h-screen flex flex-col items-center justify-center px-6 text-center gap-0">
        ...
        <p className="text-neutral-400 text-sm mb-6">Your booking is now confirmed.</p>

        <div className="w-full bg-gradient-to-br from-blue-600 to-blue-800 rounded-2xl p-5 text-left text-white mb-5 shadow-xl">
        ...
        <div className="w-full bg-orange-50 border border-orange-200 rounded-2xl p-4 flex items-center gap-3 mb-6 text-left">
          <div className="w-10 h-10 rounded-full bg-orange-500 flex items-center justify-center shrink-0">
            <CheckCircle2 className="w-5 h-5 text-white" />
          </div>
          <div>
            <p className="font-bold text-sm text-orange-800">Booking Confirmed!</p>
            <p className="text-xs text-orange-600 mt-0.5">
              {req.userName} has been notified. You'll receive trip details shortly.
            </p>
          </div>
        </div>
        ...
        <div className="w-full space-y-2">
          ...
          >
            Track My Vehicle <ChevronRight className="w-4 h-4" />
          </button>
          <button
    ...
    <div className="max-w-lg mx-auto bg-white min-h-screen pb-8">
      ...
      <div className="mx-4 mt-4 bg-gradient-to-br from-blue-600 to-blue-800 rounded-2xl p-4 text-white shadow-lg">
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-blue-500/40 text-xs text-blue-200">
          <span>Your 50% share: ₹{fare.travelerShare.toLocaleString()}</span>
          <span>Platform fee: ₹{fare.platformFee}</span>
            ...
            {selectedUpi && (
              <div className="bg-neutral-50 rounded-2xl p-4 space-y-3 mt-2">
                ...
                <p className="text-[11px] text-neutral-400">
                  e.g. mobile@okaxis · name@paytm · 9876543210@upi
                </p>
              </div>
            )}

            <div className="flex items-center gap-2 bg-blue-50 rounded-xl p-3 mt-1">
              <Smartphone className="w-4 h-4 text-blue-600 shrink-0" />
              <p className="text-xs text-blue-700">
                You'll receive a payment request on your UPI app. Approve it to complete.
              </p>
            </div>
          </>
        )}
        ...
        {tab === "card" && (
          <>
            ...
            <div className="bg-neutral-50 rounded-2xl p-4 space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full mt-1 bg-white border border-neutral-200 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-400"
                />
              </div>
              <div>
                ...
                <div className="relative mt-1">
                  <input
                    ...
                    maxLength={19}
                    className="w-full bg-white border border-neutral-200 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-400 pr-10"
                  />
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-neutral-500 font-medium">Expiry Date</label>
                  <input
                    value={card.expiry}
            ...
            <div className="flex items-center gap-2 bg-green-50 rounded-xl p-3">
              ...
              <p className="text-xs text-green-700">Your card details are encrypted with 256-bit SSL and never stored.</p>
            </div>
          </>
        )}
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <button
                  key={b}
                  onClick={() => setBank(b)}
                  className={`flex items-center gap-2 p-3 rounded-xl border text-sm font-semibold text-left transition-all active:scale-95 ${
                    bank === b
            ...
            <div className="bg-neutral-50 rounded-xl p-3">
              <p className="text-xs text-neutral-500">
                <span className="font-semibold">Other bank?</span> — Your browser will redirect to your bank's secure login page.
              </p>
              <input
                value={bank.startsWith("State Bank") || bank.startsWith("HDFC") || bank.startsWith("ICICI") || bank.startsWith("Axis") || bank.startsWith("Kotak") || bank.startsWith("Punjab") || bank.startsWith("Bank of") || bank.startsWith("Canara") ? "" : bank}
                onChange={(e) => setBank(e.target.value)}
                placeholder="Type bank name…"
                className="mt-2 w-full bg-white border border-neutral-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-200"
              />
            </div>
          </>
        )}
        ...
        >
          {processing ? (
            ...
            <>
              ...
              Pay ₹{fare.total.toLocaleString()} Securely
            </>
          )}
        </button>
